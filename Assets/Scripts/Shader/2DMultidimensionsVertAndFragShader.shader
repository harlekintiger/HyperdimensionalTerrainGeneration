...
Shader "CustomShader/2DMultidimensionsShader"
{
	Properties
	{
		_CurrentUp("Current Up", Vector) = (0, 0, 0, 0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 getNewVertPosition(float3 direction, float4 oldVert) 
			{
				return oldVert + float4(direction * calculateHeight(oldVert.xyz), 0);
			}
			...
			v2f vert (appdata v)
			{
				...
				//float3 objectOffsetDirection = mul(unity_WorldToObject, _CurrentUp.xyz);
				float3 objectOffsetDirection = mul(_CurrentUp, unity_WorldToObject).xyz;

				float4 ownVertex = float4(worldVertex + objectOffsetDirection * calculateNoiseVariant(1, worldVertex),0);
				v.vertex = mul(unity_WorldToObject, ownVertex);

				/* < Terrain Generation >		*/

				//worldVertex += float4(worldNormal * calculateHeight(worldVertex.xyz), 0);
				//float4 ownVertex = getNewVertPosition(_CurrentUp.xyz, worldVertex);
				//v.vertex = mul(unity_WorldToObject, ownVertex);
